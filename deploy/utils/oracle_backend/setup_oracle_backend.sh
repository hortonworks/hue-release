...
function usage_help {
  echo
  echo "Usage:"
  echo "USE_DEFAULT_INSTANTCLIENT=yes bash setup_oracle_backend.sh host_os"
  echo "    where host_os is one of the following:"
  echo "        centos5.9"
  echo "        centos6.4"
  echo "        suse11"
  echo
  echo "    'USE_DEFAULT_INSTANTCLIENT=yes' means to use default ORACLE_HOME and LD_LIBRARY_PATH env variables"
  echo "    what in turn implies oracle instantclient to be already installed on the system"
  echo
  echo "e.g.    USE_DEFAULT_INSTANTCLIENT=yes bash setup_oracle_backend.sh centos6.4"
}
