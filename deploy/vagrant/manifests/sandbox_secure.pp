...
class hdfs_prepare_secure {
      file { 'hdfs_prepare_secure.sh':
        path    => "/tmp/hdfs_prepare_secure.sh",
        content => template("/vagrant/files/scripts/hdfs_prepare_secure.sh"),
      }
      ...
      exec { "hdfs_prepare_secure.sh":
        command => '/bin/bash /tmp/hdfs_prepare_secure.sh |tee /var/log/hdfs_start.log',
        require => [File['hdfs_prepare_secure.sh']],
        timeout => 0,
        logoutput=> "on_failure",
      }
}
