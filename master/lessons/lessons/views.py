...
from lessons.models import Section, Step

import settings

import os, time, urllib2, string
...
def lesson(request, n, step):
	section = Section.objects.get(id=n)
	...
	return render_to_response("one_lesson.html",
			{'step':step, 'max':len(steps), 'step_html':html})

def lesson_steps(request, n, step=0):
	section = Section.objects.get(id=n)
	steps = Step.objects.filter(section=section).order_by('order')
