...
def execute_directly(request, query_msg, design=None, tablename=None,
  ...
  Note that this may throw a hcatalog exception.
  """
...
def list_designs(request):
  ...
  Depending on HCatalog configuration parameter ``SHOW_ONLY_PERSONAL_SAVED_QUERIES``,
...
def watch_query(request, id):
  ...
  if state == models.QueryHistory.STATE.expired:
    ...
    # When we fetch, HCatalog server will throw us a HCatalogException, which has the
...
def _get_server_id_and_state(query_history):
  ...
  if not server_id:
    if ok:
      raise PopupException("Query is still being submitted to the HCatalog Server")
    raise PopupException("Failed to retrieve query state from the HCatalog Server")

  state = db_utils.get_query_state(query_history)
  if state is None:
    raise PopupException("Failed to contact HCatalog Server to check query status")
