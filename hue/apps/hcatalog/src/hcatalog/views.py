...
def load_table(request, database, table):
  ...
  if request.method == "POST":
    ...
    on_success_url = urlresolvers.reverse(get_app_name(request) + ':describe_table', kwargs=dict(database=database, table=table))
    result = {'on_success_url': on_success_url}
    return HttpResponse(json.dumps(result))
  else:
  ...
  if not error:
    ...
    if downloadable:
      for format in common.DL_FORMATS:
        download_urls[format] = urlresolvers.reverse('beeswax' + ':download', kwargs=dict(id=str(id), format=format))
