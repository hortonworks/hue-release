...
if ( CodeMirror.possibleDatabase == false || CodeMirror.possibleSoloField == true) {
    if (!/^[\.\w$_]*$/.test(token.string)) {
      token = tprop = {start: cur.ch, end: cur.ch, string: "", state: token.state,
        className: token.string == "." ? "hiveql-type" : null};
    	}
	}
    if ( CodeMirror.possibleDatabase == true && CodeMirror.possibleTable == true && CodeMirror.possibleSoloField == false) {
  ...
  function getCompletions(token, context) {
    ...
    function maybeAddToExtra(str) {
      if (str.indexOf(start) == 0 && !arrayContains(found, str)) {
       if (CodeMirror.possibleTable == true && CodeMirror.possibleSoloField == false && CodeMirror.possibleDatabase == true) {
         extraFound.push(".".concat(str));
       }
       else { 
         extraFound.push(str);
       }
