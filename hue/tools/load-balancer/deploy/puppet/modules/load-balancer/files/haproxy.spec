...
Source0: http://haproxy.1wt.eu/download/1.5/src/devel/%{name}-%{version}-%{altrelease}.tar.gz
BuildRoot: %{_tmppath}/%{name}-%{version}-%{altrelease}-root
...
%prep
%setup -q -n %{name}-%{version}-%{altrelease}

# We don't want any perl dependecies in this RPM:
%define __perl_requires /bin/true

%build
%{__make} USE_PCRE=1 DEBUG="" ARCH=%{_target_cpu} TARGET=linux26 USE_OPENSSL=1 ADDLIB=-lz
