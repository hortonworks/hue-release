...
IV = lambda: os.urandom(64).encode("base64")[:16]
STATIC_IV = u'a1b2c3d4f5g6h7j8'
...
def EncryptWithAES(SECRET_KEY, plaintext_data, iv=IV()):
    encrypt_cipher = AES.new(SECRET_KEY, AES.MODE_CBC, iv)
...
def DecryptWithAES(SECRET_KEY, encrypted_data, iv=IV()):
